    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 p-4 md:p-8">
        <div className="mx-auto w-full max-w-6xl grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-6 md:gap-8">
          <section className="rounded-3xl bg-card border border-border shadow-sm p-6 md:p-8">
            <div className="mb-8 space-y-2">
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#7BDFF2]">Dailies</p>
              <h1 className="text-3xl md:text-4xl font-[family-name:var(--font-overlock)] font-bold text-card-foreground">How are you feeling right now?</h1>
              ...
              {prompts.map((prompt) => (
                <div key={prompt.label} className="space-y-2">
                  <div className="flex items-end justify-between gap-4">
                    <p className="text-lg text-foreground">{prompt.label}</p>
                    <p className="text-xs text-muted-foreground">{prompt.helper}</p>
                  </div>
                  <input
                    type="range"
                    min="0"
          ...
          <aside className="rounded-3xl bg-[color:var(--surface-pink)] border border-border p-6 md:p-8 flex flex-col gap-6">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#F2B5D4]">Multimodal</p>
              <h2 className="text-2xl font-[family-name:var(--font-overlock)] font-bold text-foreground mt-2">Quick Photo</h2>
              <p className="text-sm text-muted-foreground mt-2">
                Capture your environment, a note, or a moment to help Sarah understand context.
              </p>
            </div>

            <div className="rounded-2xl border-2 border-dashed border-[#F2B5D4]/50 bg-card/70 min-h-44 flex items-center justify-center px-6 text-center text-muted-foreground text-sm">
