...
const SUGGESTED_PROMPTS = [
  ...
  "I'm struggling to sleep and can't switch off.",
        ...
        <header className="flex items-center justify-between bg-card/90 backdrop-blur-md p-4 rounded-3xl border border-border shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#B2F7EF] flex items-center justify-center border border-[#7BDFF2]/40">
              <Image src="/Logo.svg" alt="Rooftop" width={20} height={20} className="opacity-70 dark:invert" />
            </div>
            <div>
              <h1 className="font-semibold text-card-foreground leading-none">Rooftop</h1>
        ...
        <Card className="flex-1 flex flex-col bg-card/80 backdrop-blur-md border-border shadow-xl rounded-[32px] overflow-hidden">
            ...
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-6 py-12">
                ...
                <div className="space-y-2">
                  <h3 className="text-xl font-semibold text-foreground">How are you feeling today?</h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-md">
                  {SUGGESTED_PROMPTS.map((prompt) => (
                    <button
                      key={prompt}
                      onClick={() => send(prompt)}
                      className="text-left p-3 rounded-2xl border border-border bg-muted/50 hover:bg-muted text-sm text-foreground/80 hover:text-foreground transition-colors"
                    >
                      {prompt}
                    </button>
                  ))}
          ...
          <div className="p-4 md:p-6 bg-muted/50 border-t border-border">
            ...
            <p className="text-[10px] text-center text-muted-foreground mt-3 uppercase tracking-widest font-medium">
              Confidential · Not a substitute for professional care
            </p>
          </div>
        </Card>
      </div>
